...
export const VoiceInputModal = ({
  ...
  const processAudio = async () => {
    ...
    try {
      ...
      // Then process the transcript to extract expense data
      const processingResponse = await fetch("/api/process-voice-expense", {
        method: "POST",
        headers: {
          ...
          <div className="p-4 bg-gray-700 rounded-lg min-h-32 flex items-center justify-center">
            {isRecording ? (
              <div className="flex items-center gap-2">
                <div className="h-4 w-1 bg-red-500 animate-pulse"></div>
                <div className="h-6 w-1 bg-red-500 animate-pulse"></div>
                <div className="h-4 w-1 bg-red-500 animate-pulse"></div>
